...
def get_sum(page):
    url = 'http://match.yuanrenxue.com/api/match/4?page=%s' %page
    ...
    key_value = response.json()['key'].encode('utf-8')+response.json()['value'].encode('utf-8')
    key_value = base64.b64encode(key_value).decode('utf-8').replace('=', '').encode('utf-8')
    print(hashlib.md5(key_value).hexdigest())
    cls = hashlib.md5(key_value).hexdigest()
    tab_imgs = response.json()['info']
    imgs = re.findall('<td>(.*?)</td>',tab_imgs)

    num = []
    num_list = []
    for img in imgs:
        im= re.findall('<img(.*?)>',img)
        for i in im:
            if cls not in i:
                key = re.findall('src="(.*?)"',i)[0]
                left = re.findall('style="left:(.*?)px"',i)[0]
                left = float(left)
                num.append((dict_num[key],int(left/11.5)))
            else:
                continue
        num_list.append(num)
        num = []
    ...
    for new in num_list:
        ...
        for index,i in enumerate(new):
            num[index+i[1]] = str(new[index][0])

        new_num.append(int(''.join(num)))
    print(new_num)
    return sum(new_num)

sums = sum([get_sum(i+1) for i in range(5)])
print('5页的全部数字加和:',sums)
