...
FILENAME = 'AnHui_weather_data.txt'


# 获取安徽省所有城市的区域ID
def get_AnHui_areaid():
    try:
        # 使用requests库发送请求
        ...
        return []
    # 提取安徽省所有城市的区域ID
    AnHui_area_ids = [data.get('AREAID') for city, counties in city_data.get("广东", {}).items() for county, data in
                      counties.items() if data.get('AREAID')]
    return AnHui_area_ids
    ...
    try:
        ...
        with open(filename, 'w', encoding='utf-8') as file:
            ...
            for data in weather_data:
                ...
                # 将天气数据写入文件
                file.write(
                    f"日期:{date:<10}天气情况:{weather:<9}最低温度:{low_temp:<9}最高温度:{high_temp:<9}风向:{wind:<15}等级:{level:<6}\n")
        # 打印消息，表示数据已经写入文件
...
# 主函数，获取安徽省所有城市的天气数据，并写入文件
def main():
    try:
        AnHui_area_ids = get_AnHui_areaid()
        all_weather_data = []
        for area_id in AnHui_area_ids:
            print(f"Getting weather data for {area_id}")
